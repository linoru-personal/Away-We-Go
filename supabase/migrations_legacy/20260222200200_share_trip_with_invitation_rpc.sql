-- share_trip_with_invitation: direct membership if email has an account, else pending invitation.
-- Does not redesign existing permissions. Uses can_manage_trip_sharing; access remains via trip_members only.
-- Requires: pgcrypto (gen_random_bytes, digest). Supabase has it by default.
...
begin
  ...
  -- Generate single-use token for accept link; store only hash in DB
...
comment on function public.share_trip_with_invitation(uuid, text, text) is
  'Share trip by email: add to trip_members if user exists, else create/refresh pending invitation. Return outcome + invitation_token for server to send email.';

grant execute on function public.share_trip_with_invitation(uuid, text, text) to authenticated;
