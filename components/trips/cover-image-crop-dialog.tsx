...
export interface CoverImageCropDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  imageSrc: string;
  /** When null (create mode), crop result is returned via onCropComplete. When set (edit mode), dialog uploads and calls onSuccess. */
  tripId: string | null;
  onSuccess?: () => void;
  /** Called with cropped blob when tripId is null (create mode). */
        ...
        <div className="flex max-h-[90vh] min-h-0 w-full flex-col">
          <div className="shrink-0">
            <DialogHeader>
              ...
              <p className="mt-1 text-[15px] leading-relaxed text-[#6b6b6b]">
                Adjust the crop (wide format for your trip cover). Then {tripId ? "save" : "continue"}.
              </p>
            </DialogHeader>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            <div className="mt-4 flex gap-3 pb-1">
              ...
              >
                {uploading ? "Uploading…" : tripId ? "Save" : "Done"}
              </button>
            </div>
          </div>
        </div>
